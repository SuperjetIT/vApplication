...
import { isUserLoggedIn } from '../utils/authGate'
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '0 0 6px', fontSize: 13, opacity: 0.95 }}>
          If your visa is delayed — we follow up with the embassy and keep you updated
        </p>
...
export default function VisaPage() {
  ...
  const [authGateOption, setAuthGateOption] = useState<{ applyUrl: string; visaOption: string } | null>(null)
  ...
  const handleApplyAttempt = (optionId: string) => {
    const url = buildApplyUrl(optionId)
    const option = country.visaOptions.find((o) => o.id === optionId) ?? selectedOption
    if (isUserLoggedIn()) {
      navigate(url)
      return
    }
    setAuthGateOption({
      applyUrl: url,
      visaOption: option?.label ?? 'Single Entry Visa',
    })
    setShowAuthGate(true)
  }

  const handleSecondaryApply = () => {
    const otherOption = country.visaOptions.find((o) => o.id !== selectedOptionId)
    if (!otherOption) return
    setSearchParams({ option: otherOption.id }, { replace: true })
  ...
  const paymentBox = isOwnCountry ? null : (
    <PaymentBox
      ...
      onApply={() => handleApplyAttempt(selectedOptionId)}
      onSecondaryApply={handleSecondaryApply}
    />
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafafa', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {showAuthGate && authGateOption && (
        <AuthGateModal
          isOpen={showAuthGate}
          onClose={() => setShowAuthGate(false)}
          destinationCountry={country.name}
          countryFlag={country.countryCode}
          visaOption={authGateOption.visaOption}
          applyUrl={authGateOption.applyUrl}
        />
      )}
    </div>
  )
