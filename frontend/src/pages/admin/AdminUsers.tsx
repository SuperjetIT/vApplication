...
export default function AdminUsers() {
  ...
  const [showDelete, setShowDelete] = useState<OperationStaffUser | null>(null)
  ...
  const handleDelete = () => {
    ...
    if (isSuperAdminUser(showDelete)) {
      setToast('Super Admin account cannot be deleted')
      closeDeleteDialog()
      return
    }
    const result = deleteOperationUser(showDelete.id)
    if (!result.ok) {
      setToast(result.reason === 'protected' ? 'This account cannot be deleted' : 'User not found')
      closeDeleteDialog()
      return
    }
    setUsers((current) => {
      const admin = current.find((x) => isSuperAdminUser(x))
      const remaining = current.filter((u) => u.id !== showDelete.id && !isSuperAdminUser(u))
      return admin ? [admin, ...remaining] : remaining
    })
    const deletedName = showDelete.name
    closeDeleteDialog()
    setToast(`${deletedName} has been removed`)
  }
  ...
  const openDeleteDialog = (user: OperationStaffUser) => {
    setShowDelete(user)
    setDeleteConfirmStep(1)
  }
  ...
  return (
    <AdminLayout activePath="/admin/users" title="Operation Users">
      ...
      {showDelete && (
        <>
          <div role="presentation" onClick={closeDeleteDialog} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 2000 }} />
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', ...cardStyle, padding: 32, maxWidth: 420, width: '90%', zIndex: 2001 }}>
            {deleteConfirmStep === 1 ? (
              <>
                <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 700, color: TEXT_PRIMARY }}>Delete operation user?</h3>
                <p style={{ fontSize: 13, color: TEXT_SECONDARY, marginBottom: 8 }}>
                  You are about to remove <strong>{showDelete.name}</strong> ({showDelete.email}).
                </p>
                <p style={{ fontSize: 13, color: '#b91c1c', marginBottom: 24, lineHeight: 1.5 }}>
                  They will lose access to the operations portal immediately, even if they still know the URL and password.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={closeDeleteDialog} style={{ ...outlineBtn, flex: 1 }}>Cancel</button>
                  <button
                    type="button"
              ...
              <>
                <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 700, color: TEXT_PRIMARY }}>Are you sure?</h3>
                <p style={{ fontSize: 13, color: TEXT_SECONDARY, marginBottom: 8, lineHeight: 1.5 }}>
                  Are you sure you want to delete <strong>{showDelete.name}</strong>? This cannot be undone.
                </p>
                <p style={{ fontSize: 13, color: '#b91c1c', marginBottom: 24, lineHeight: 1.5 }}>
                  Type of access removed: operations portal login for <strong>{showDelete.username}</strong>.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setDeleteConfirmStep(1)} style={{ ...outlineBtn, flex: 1 }}>Go back</button>
                  <button
                    type="button"
