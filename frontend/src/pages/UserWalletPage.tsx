...
function AddMoneyForm({ userId, onSuccess }: { userId: string; onSuccess: (amount: number) => void }) {
  ...
  const handleAdd = async () => {
    ...
    if (isStripeMockCheckout() || !stripe) {
      window.setTimeout(() => {
  ...
  return (
    <div>
      <p style={{ margin: '0 0 16px', fontSize: 13, color: '#64748b', lineHeight: 1.5 }}>
        Choose an amount and pay by card. Funds are added to your wallet instantly and can be used at visa checkout.
      </p>
      <label style={{ display: 'block', fontSize: 13, color: '#666', marginBottom: 8 }}>Recharge amount (AED)</label>
      <input
        type="number"
        min={1}
        value={amount}
      ...
      {!isStripeMockCheckout() ? (
        <div style={{ padding: 14, border: '1px solid #e5e7eb', borderRadius: 12, marginBottom: 16, background: '#fff' }}>
          <p style={{ margin: '0 0 10px', fontSize: 12, fontWeight: 600, color: '#374151' }}>Card details</p>
          ...
          <p style={{ margin: '10px 0 0', fontSize: 11, color: '#888', textAlign: 'center' }}>
            🔒 Secured by Stripe{isStripeTestMode() ? ' (test mode)' : ''}
          </p>
        </div>
      ) : (
        <div
        ...
        >
          <strong style={{ color: '#1e40af' }}>Dev demo mode</strong> — card form is skipped locally.
          {isStripeConfigured() ? ' Set VITE_STRIPE_MOCK_CHECKOUT=false to test real Stripe card entry.' : ' Enable Stripe in Admin → Settings and add your publishable key, or set VITE_STRIPE_PUBLISHABLE_KEY in .env.'}
          {' '}Click below to simulate a successful recharge.
        </div>
      )}
      ...
      <button type="button" disabled={paying || amount <= 0} onClick={handleAdd} style={{ width: '100%', background: B2C_ACCENT, color: '#fff', border: 'none', borderRadius: 12, padding: 14, fontWeight: 700, cursor: 'pointer', opacity: paying ? 0.7 : 1 }}>
        {paying ? 'Processing…' : isStripeMockCheckout() ? `Recharge AED ${amount.toLocaleString()} (demo)` : `Pay AED ${amount.toLocaleString()} with Card`}
      </button>
    </div>
  )
}
      ...
      <main style={{ flex: 1, maxWidth: 800, margin: '0 auto', padding: '32px 24px', width: '100%' }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${B2C_ACCENT}, #818cf8)`, borderRadius: 24, padding: 40, color: '#fff', textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24, flexWrap: 'wrap' }}>
            <button type="button" onClick={() => setShowAddModal(true)} style={{ background: '#fff', color: B2C_ACCENT, border: 'none', borderRadius: 12, padding: '12px 24px', fontWeight: 700, cursor: 'pointer' }}>Recharge with Card</button>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24 }} onClick={() => setShowAddModal(false)}>
          <div style={{ background: '#fff', borderRadius: 20, padding: 28, maxWidth: 420, width: '100%' }} onClick={(e) => e.stopPropagation()}>
            <h2 style={{ margin: '0 0 8px', fontSize: 18 }}>Recharge Wallet</h2>
            <p style={{ margin: '0 0 20px', fontSize: 13, color: '#64748b' }}>Pay by card to add balance to your wallet.</p>
