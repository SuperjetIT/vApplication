...
export const Database = {
  ...
  getAdminNotifications: () =>
    [...db.notifications]
      .filter((n) => String(n.userType) === 'admin' || String(n.userId) === 'admin')
      .sort(
        (a, b) =>
  ...
  getAdminUnreadNotificationCount: () =>
    db.notifications.filter(
      (n) =>
        (String(n.userType) === 'admin' || String(n.userId) === 'admin') && n.read !== true,
    ).length,
  createAdminNotification: (data: GenericRecord) => {
    const newNotif = {
      id: generateId('notif'),
      userId: 'admin',
      userType: 'admin',
  ...
  markAllAdminNotificationsRead: () => {
    ...
    db.notifications = db.notifications.map((n) => {
      if (String(n.userType) !== 'admin' && String(n.userId) !== 'admin') return n
