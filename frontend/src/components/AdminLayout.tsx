...
export function AdminLayout({ activePath, title, children }: { activePath: string; title: string; children: ReactNode }) {
  ...
  const adminNotifications = useMemo(() => Database.getAdminNotifications(), [dbVersion])
  const unreadCount = useMemo(
    () => adminNotifications.filter((n) => n.read !== true).length,
    [adminNotifications],
  )
  ...
  const handleMarkAllNotificationsRead = () => {
    Database.markAllAdminNotificationsRead()
  }
          ...
          <div className="admin-header-actions">
            ...
            <div style={{ position: 'relative' }}>
              <button type="button" onClick={toggleNotifications} style={{ width: 40, height: 40, borderRadius: '50%', background: PAGE_BG, border: `1px solid ${BORDER}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                ...
                {unreadCount > 0 && (
                  <span style={{ position: 'absolute', top: 4, right: 4, minWidth: 16, height: 16, borderRadius: 8, background: BRAND, border: '2px solid white', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>
                    {unreadCount > 9 ? '9+' : unreadCount}
                  </span>
                )}
              </button>
              {notifOpen && (
                <div className="admin-notif-panel" style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.12)', border: `1px solid ${BORDER}`, width: 360, maxHeight: 480, zIndex: 9999, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: `1px solid ${BORDER}`, flexShrink: 0 }}>
                    <span style={{ fontWeight: 700, color: TEXT_PRIMARY }}>Notifications{unreadCount > 0 ? ` (${unreadCount})` : ''}</span>
                    {unreadCount > 0 && (
                      <button type="button" onClick={handleMarkAllNotificationsRead} style={{ border: 'none', background: 'none', color: BRAND, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>Mark all read</button>
                    )}
                  </div>
                  <div style={{ overflowY: 'auto', flex: 1 }}>
                    {adminNotifications.length === 0 ? (
                      <div style={{ padding: 32, textAlign: 'center', color: TEXT_MUTED, fontSize: 13 }}>No notifications yet</div>
                    ) : (
                      adminNotifications.map((n) => {
                        const unread = n.read !== true
                        const category = String(n.category ?? '')
                        const paymentStatus = String(n.paymentStatus ?? '')
                        const gradient = getNotificationGradient(category, paymentStatus)
                        const amountPending = Number(n.amountPending ?? 0)
                        const whatsappMessage = String(n.whatsappMessage ?? '')
                        const customerPhone = String(n.customerPhone ?? '')
                        return (
                          <div
                            key={String(n.id)}
                            role="button"
                            ...
                            style={{ display: 'flex', gap: 12, padding: 16, borderBottom: '1px solid #f5f5f5', alignItems: 'flex-start', cursor: 'pointer', background: unread ? '#fafbff' : '#fff' }}
                          >
                            {unread ? (
                              <span style={{ width: 6, height: 6, borderRadius: '50%', background: isOperations ? OPS_PRIMARY : BRAND_BLUE, marginTop: 8, flexShrink: 0 }} />
                            ) : (
                              <span style={{ width: 6, flexShrink: 0 }} />
                            )}
                            <div style={{ width: 40, height: 40, borderRadius: '50%', background: gradient, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" /></svg>
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: 600, fontSize: 13, color: TEXT_PRIMARY }}>{String(n.title ?? 'Notification')}</div>
                              <div style={{ fontSize: 12, color: TEXT_SECONDARY, marginTop: 2, lineHeight: 1.45 }}>{String(n.message ?? '')}</div>
                              {amountPending > 0 && (
                                <div style={{ display: 'inline-block', marginTop: 6, fontSize: 11, fontWeight: 600, color: '#b45309', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 6, padding: '2px 8px' }}>
                                  Amount pending: AED {amountPending.toLocaleString()}
                                </div>
                              )}
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                                <div style={{ fontSize: 11, color: TEXT_MUTED }}>{formatNotificationTimeAgo(String(n.createdAt ?? new Date().toISOString()))}</div>
                                {whatsappMessage && (
                                  <a
                                    href={buildSupportWhatsAppUrl(whatsappMessage, customerPhone || undefined)}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    onClick={(e) => e.stopPropagation()}
                                    style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, color: '#16a34a', textDecoration: 'none', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 6, padding: '3px 8px' }}
                                  >
