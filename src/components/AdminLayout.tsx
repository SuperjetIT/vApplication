...
const NAV_SECTIONS = [
  ...
  { label: 'CRM', items: [{ path: '/admin/leads', label: 'Leads' }, { path: '/admin/customers', label: 'Customers' }, { path: '/admin/agents', label: 'Agents' }] },
...
export function AdminLayout({ activePath, title, children }: { activePath: string; title: string; children: ReactNode }) {
  ...
  const handleLogout = () => { logout(); navigate(ADMIN_LOGIN_PATH) }
  ...
  const handleHeaderSearch = (e: FormEvent) => {
    ...
    if (!q) return
    navigate(`/admin/leads?search=${encodeURIComponent(q)}`)
  }
  ...
  const isActive = (path: string, label: string) => {
    ...
    if (label === 'Leads') return activePath === '/admin/leads' || activePath.startsWith('/admin/cases')
  ...
  return (
    <div style={{ minHeight: '100vh', background: PAGE_BG, color: TEXT_PRIMARY }}>
      <style>{`@media (max-width:768px){.admin-hamburger{display:flex!important}.admin-sidebar-wrap{transform:translateX(-100%);transition:transform .25s}.admin-sidebar-wrap.open{transform:translateX(0)!important}.admin-main{margin-left:0!important}.admin-header-search{display:none!important}.admin-wallet-pills{display:none!important}}`}</style>
        ...
        <aside style={{ width: SIDEBAR_W, height: '100vh', background: SIDEBAR_BG, borderRight: `1px solid ${BORDER}`, boxShadow: '2px 0 8px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #f93e42, #ff6b6b)', boxShadow: '0 4px 12px rgba(249,62,66,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 16 }}>SV</div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 18, color: TEXT_PRIMARY, lineHeight: 1.2 }}>Super Visa</div>
                <span style={{ background: '#fff0f0', color: BRAND, borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 600 }}>Admin</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ margin: 16, borderRadius: 16, background: 'linear-gradient(135deg, #f8f9fc, #f0f4ff)', border: `1px solid ${BORDER}`, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ position: 'relative' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #f93e42, #ff6b6b)', color: '#fff', fontWeight: 700, fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(249,62,66,0.3)' }}>SA</div>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: TEXT_PRIMARY }}>Super Admin</div>
                <div style={{ color: TEXT_SECONDARY, fontSize: 12 }}>Administrator</div>
              </div>
            </div>
      ...
      <div className="admin-main" style={{ marginLeft: SIDEBAR_W, minHeight: '100vh' }}>
        <header style={{ background: '#fff', borderBottom: `1px solid ${BORDER}`, height: 68, padding: '0 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', position: 'sticky', top: 0, zIndex: 100 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button type="button" className="admin-hamburger" onClick={() => setMobileOpen((o) => !o)} style={{ display: 'none', border: 'none', background: PAGE_BG, borderRadius: 10, cursor: 'pointer', padding: 8 }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={TEXT_PRIMARY} strokeWidth="2"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
            </button>
            <div>
              <div style={{ fontSize: 13, color: TEXT_MUTED }}>Admin / {title}</div>
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: TEXT_PRIMARY }}>{title}</h1>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {notifOpen && (
                <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.12)', border: `1px solid ${BORDER}`, width: 340, zIndex: 9999, overflow: 'hidden' }}>
            ...
            <span style={{ color: TEXT_SECONDARY, fontSize: 13 }}>{today}</span>
            <div style={{ position: 'relative' }}>
              <button type="button" onClick={() => { setProfileOpen((o) => !o); setNotifOpen(false) }} style={{ display: 'flex', alignItems: 'center', gap: 8, background: PAGE_BG, border: `1px solid ${BORDER}`, borderRadius: 40, padding: '6px 14px 6px 6px', cursor: 'pointer' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #f93e42, #ff6b6b)', color: '#fff', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>SA</div>
                <span style={{ fontSize: 13, fontWeight: 600, color: TEXT_PRIMARY }}>Super Admin</span>
...
export function AdminGuard({ children }: { children: ReactNode }) {
  if (localStorage.getItem('admin_logged_in') !== 'true') return <Navigate to={ADMIN_LOGIN_PATH} replace />
  return children
}
