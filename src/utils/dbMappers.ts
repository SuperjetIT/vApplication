...
function travelerNationality(travelers: unknown): string {
  if (!Array.isArray(travelers) || travelers.length === 0) return '—'
  return String((travelers[0] as GenericRecord).nationality ?? '—')
}
...
export function applicationToLead(app: GenericRecord): AdminLead {
  ...
  let email = ''
  let passport = travelerNationality(travelers)
  let passportCode = 'ae'
  ...
  if (app.userId) {
    ...
    if (user) {
      ...
      if (user.passportCountry) passport = String(user.passportCountry)
    }
  ...
  return {
    ...
    email,
    passport,
    passportCode: passportCode || destSlug.slice(0, 2) || 'ae',
